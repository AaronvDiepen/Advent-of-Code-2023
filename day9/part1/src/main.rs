...
fn process_line(line: String) -> i64 {
    ...
    first_row.iter().enumerate()
        .map(|(i, entry)| {
            if i % 2 == 0 {
                // For even indexed entries we need to add the entry times the value from pascals triangle
                binomial(first_row.len(), i) as i64 * entry
            } else {
                // For odd indexed entries we need to subtract the entry times the value from pascals triangle
                -1 * binomial(first_row.len(), i) as i64 * entry
            }
        })
